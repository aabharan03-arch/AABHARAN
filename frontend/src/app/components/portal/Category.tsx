...
const CACHE_KEY = 'storeadmin_categories_cache';
const CACHE_TIME_KEY = 'storeadmin_categories_cache_time';
...
export default function CategoriesPage() {
  ...
  // Fetch Categories with Cache Handling
  ...
  // Derived memoized categories list
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        updatedCategories = [data.category, ...categories];
      }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[#FAF8F5] text-[#1A1A1A] font-sans relative">
      ...
      <header className="sticky top-0 z-30 bg-[#FAF8F5]/80 backdrop-blur-md border-b border-amber-900/5 px-10 py-6 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all">
        <div>
          <h1 className="text-3xl font-serif text-[#121625] font-normal">Categories</h1>
          ...
          <div className="bg-white rounded-2xl border border-amber-900/10 p-12 text-center shadow-sm max-w-2xl mx-auto mt-6">
            ...
            <p className="text-sm text-slate-500 mb-6 max-w-sm mx-auto">
              {searchQuery
                ...
                : 'Get started by creating your first ornament category (e.g., Rings, Necklaces, Bangles).'}
            </p>
            {searchQuery ? (
              <button
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl border border-amber-900/10 overflow-hidden p-6 text-center">
            ...
            <p className="text-sm text-slate-500 mb-6 leading-relaxed">
              Are you sure you want to delete <span className="font-semibold text-slate-800">"{categoryToDelete.name}"</span>? This action cannot be undone.
            </p>
